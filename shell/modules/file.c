...
static void table2tm( lua_State *L, vfs_time *tm )
{
  ...
  tm->year = luaL_optinteger( L, ++idx, 2016 );
  tm->mon  = luaL_optinteger( L, ++idx, 6 );
  tm->day  = luaL_optinteger( L, ++idx, 21 );
  tm->hour = luaL_optinteger( L, ++idx, 0 );
  tm->min  = luaL_optinteger( L, ++idx, 0 );
  tm->sec  = luaL_optinteger( L, ++idx, 0 );
...
static int file_seek (lua_State *L)
{
  ...
  long offset = luaL_optinteger(L, 2, 0);
